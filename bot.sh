...
set_api_url() {
    ...
    if [ "$system_type" -eq 0 ]; then
        # VPS
        API_URL="https://zeddead.gaia.domains/v1/chat/completions"
        API_NAME="dead1"
...
generate_random_general_question() {
    if [[ "$API_URL" == "https://zeddead.gaia.domains/v1/chat/completions" ]]; then
general_questions=(
    ...
    elif [[ "$API_URL" == "https://zeddead.gaia.domains/v1/chat/completions" ]]; then
general_questions=(
...
EOF
    ...
    if [[ "$API_URL" == "https://zeddead.gaia.domains/v1/chat/completions" ]]; then
        echo "⏳ Fetching (dead1 API)..."
        sleep 1
    elif [[ "$API_URL" == "zeddead.gaia.domains/v1/chat/completions" ]]; then
        echo "⏳ Fetching (dead2 API)..."
        sleep 2
    elif [[ "$API_URL" == "https://zeddead.gaia.domains/v1/chat/completions " ]]; then
        echo "⏳ Fetching..."
        sleep 1
